...
export async function POST(request: Request) {
    try {
        ...
        const { action, boardingId, problemTypeId, description } = await request.json();
        ...
        // Validar que boardingId esté presente
        if (!boardingId) {
            return new Response("Falta el parámetro 'boardingId'", { status: 400 });
        }
...
export async function PATCH(request: Request) {
    try {
        const { action, boardingId, issueId, validatorId, downloadEndDate, validationStartDate, validationEndDate, rampId, forkliftOperatorId, downloadStartDate, pallets, assistantId, captureStartDate, captureEndDate, completedDate, } = await request.json();
        ...
        if (action === "resolveProblem") {
            ...
            // Actualizar el problema como resuelto
            ...
            // Verificar problemas restantes asociados al mismo `boardingId`
            ...
            // Si no quedan problemas, actualizar el estado `hasIssues`
        ...
        else if (action === "assignRamp") {
            // Validaciones y lógica para asignar la rampa
            if (!boardingId || !rampId || !downloadStartDate) {
                return new Response("Faltan parámetros", { status: 400 });
            }

            // Verificar si el embarque tiene problemas (hasIssues: true)
            const boardingHasIssues = await prisma.boarding.findUnique({
                where: { id: boardingId },
                select: { hasIssues: true },
            });

            if (!boardingHasIssues) {
                return new Response("El embarque no existe", { status: 404 });
            }

            if (boardingHasIssues.hasIssues) {
                return new Response(
                    "El embarque tiene problemas reportados. Resuélvelos antes de continuar.",
                    { status: 400 }
                );
            }
