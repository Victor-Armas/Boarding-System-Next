...
import { updateBoarding } from "@/actions/update-boarding-action"; // Importa la función de actualización
import { PencilSquareIcon, CalendarIcon, UserIcon, ArchiveBoxIcon, CheckCircleIcon, XCircleIcon } from '@heroicons/react/24/outline';
...
export default function EditBoardingForm({ boardingId, initialValues }: EditBoardingFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...initialValues,
    dateTime: initialValues.dateTime ? initialValues.dateTime.toISOString().slice(0, 16) : "", // Formato 'yyyy-MM-ddTHH:mm'
  });
  ...
  const validateForm = () => {
    ...
    if (!formData.numberBox) newErrors.numberBox = "El número de caja es obligatorio.";
